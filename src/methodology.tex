...
Signed distance field, shown on figure~\ref{fig:sdf}, has the same dimensions as the initial binary mask. The \ac{SDF} value $v$ at pixel $p$ is defined as the sum of these two distance transforms
\begin{equation}
...
The user is given option to adjust both Gaussian blur parameter $\sigma$ the threshold value. Increasing the threshold erodes the resulting mask, shown on figure~\ref{fig:binary_mask_eroded}, whereas decreasing it expands the foreground region, shown on figure~\ref{fig:binary_mask_expanded}.
...
\begin{figure}[htbp]
	...
	\caption{
		Example of resulting binary mask skeleton without (a) smoothing and with smoothing (b). The gray area is the binary mask, either initial or smoothed. Smoothing was performed with $\sigma = 15.0$ and threshold $-1.2$.
	}
\end{figure}
...
A cubic B\'ezier curve $\mathbf{B}(t)$, shown on figure~\ref{fig:bezier_curve}, is defined by two end points, $\mathbf{P}_0$ and $\mathbf{P}_3$, and two control points, $\mathbf{P}_1$ and $\mathbf{P}_2$. It is given by
\begin{equation}
...
\begin{figure}[htbp]
	...
	\caption{Example of cubic B\'ezier curve constructed from four control points.}
	\label{fig:bezier_curve}
\end{figure}

A Hermite curve $\mathbf{H}(t)$, shown on figure~\ref{fig:hermite_spline}, is defined by two end points, $\mathbf{H}_0$ and $\mathbf{H}_1$, and two tangent vectors, $\mathbf{M}_0$ and $\mathbf{M}_1$. It is given by
\begin{equation}
...
\begin{figure}[htbp]
	...
	\caption{Example of Hermite spline constructed from two points and two tangent vectors.}
...
\end{equation}
where $\alpha \in [0,1]$ and $t_{0} = 0$. If $\alpha = 0$ then the parameterization is uniform, for $\alpha = 1$ the parameterization becomes chordal. Example of spline interpolation through polyline defined by points $\mathbf{P_0}$ to $\mathbf{P_5}$ can be seen on figure~\ref{fig:catmull_rom}.
...
\begin{figure}[htbp]
	...
	\caption{Example of spline interpolation by Catmull--Rom spline. Points $\mathbf{B}_1,\mathbf{B}_2,\mathbf{C}_1,\mathbf{C}_2,\mathbf{D}_1,\mathbf{D}_2$ are corresponding control points for B\'ezier curve.}
...
\end{equation}
Final point $\mathbf{S}$ lies on a cubic B\'ezier $\mathbf{B}(t)$ and is equal to a point $\mathbf{B}(u)$. Algorithm is visualized on figure\ref{fig:de_casteljau}

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.6\textwidth]{./fig/geogebra/de_casteljau.pdf}
	\caption{Visual example of de Casteljau algorithm for $u=0.35$.}
	\label{fig:de_casteljau}
\end{figure}

We can then create two cubic B\'ezier curves $\mathbf{B}_A(t_A)$ and $\mathbf{B}_B(t_B)$, and $t_A,t_B \in \left[0,1\right]$. B\'ezier curve $\mathbf{B}_A$, visualized in figure~\ref{fig:de_casteljau_subdivision_a}, is defined by control points $\mathbf{P}_0$, $\mathbf{Q}_0$, $\mathbf{R}_0$ and $\mathbf{S}$. B\'ezier curve $\mathbf{B}_B$, visualized in figure~\ref{fig:de_casteljau_subdivision_b}, is defined by control points $\mathbf{S}$, $\mathbf{R}_1$, $\mathbf{Q}_2$ and $\mathbf{P}_3$. These segments when combined results in the initial B\'ezier curve $\mathbf{B}(t)$. 
...
\begin{figure}[htbp]
	...
	\caption{
		Example of subdividing B\'ezier curve using de Casteljau algorithm for $u=0.35$, the left subdivion (a) and the right subdivision (b).
	}
\end{figure}
