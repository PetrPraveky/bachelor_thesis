...
The Euclidean distance transform, referred to as EDT, of a binary image assigns to each pixel the Euclidean distance to the nearest specified pixel. This can be used to transform widths and distances from image space into real-world units.

In our implementation, we use the transformation of a cost function on a grid using spatial information~\cite{bk_edt}. This formulation is a generalization of the Euclidean distance transform for binary images. To compute distances assigned to individual pixels, we define the cost function as
\begin{equation}
	f(q) =
	\begin{cases}
		0 & \text{if } q \text{ is a foreground pixel}, \\
		\infty & \text{otherwise},
	\end{cases}
\end{equation}
...
% todo obrázek binarymask -> hodnoty EDT (+ nekonečna?)
...
An SDF is a field in which the value corresponding to each pixel represents the distance from that pixel to the nearest edge of a foreground region. To compute the EDT for background pixels, we invert the cost function $f(q)$, i.e.,
\begin{equation}
	f(q) =
	\begin{cases}
		0 & \text{if } q \text{ is a background pixel}, \\
		\infty & \text{otherwise},
	\end{cases}
\end{equation}
where $q$ is a pixel of the binary mask. The resulting value is taken as negative for background pixels and positive for foreground pixels.
