...
A complete procedural terrain generation method was proposed in~\cite{bk_procedural_terrain_gan}. The method uses a \ac{CAN} to procedurally generate satellite images. The generated satellite image, or alternatively an existing satellite image as in the real-world-data-based approaches discussed above, is then used as input to a \ac{CGAN} to generate the corresponding \ac{DEM}. These outputs can be used to define both the shape and the texture of the final terrain. However, the proposed method focuses only on terrain generation and does not populate the environment with elements such as foliage, roads, rivers, or buildings.
